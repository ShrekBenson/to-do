...
import { editTask } from '../functions/edit-task';
...
class dialogEditRender{
  ...
  render(title, description, dueDate, priority){
    ...
    dialogBarTitle.innerText = 'Task edition';
    ...
    dialogButton.innerText = 'Update';

    if(priority === 'secundary'){
      priorityLow.classList.add('select');
    } else if(priority === 'urgent'){
      priorityHigh.classList.add('select');
    }
    ...
    dialogButton.addEventListener('click', () => {
      let prioritySelect = prioritySelected(priorityLow, priorityHigh);

      if (requiredFields(titleField, descriptionField)) {
        errorAlert(titleField, descriptionField);
      } else{      
        editTask(this.key, titleField.value, descriptionField.value, dueDateField.value, prioritySelect);        
        closeDialog(dialogForm);
      }
    })
